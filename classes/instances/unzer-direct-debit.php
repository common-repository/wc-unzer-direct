...
class WC_UnzerDirect_Debit extends WC_UnzerDirect_Instance {
	...
	public function __construct() {
		...
		$this->id = 'unzer_direct_debit';

		$this->method_title = 'Unzer Direct Debit';
	...
	public function init_form_fields() {
		$this->form_fields = [
			...
			'description' => [
				...
				'default'     => __( 'Pay directly from your bank account', 'wc-unzer-direct' ),
			],
		];
	}
	...
	public function maybe_disable_gateway( $gateways ) {
		if ( isset( $gateways[ $this->id ] ) && is_checkout() && ( $cart = WC()->cart ) ) {
			...
			if ( ! ( $cart_total >= $cart_min && $cart_total <= $cart_max ) || ( 'EUR' !== $currency && 'CHF' !== $currency ) || ( 'DE' !== $country && 'AT' !== $country ) || $this->is_checkout_different_shipping_address_enabled() ) {
