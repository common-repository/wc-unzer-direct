...
class WC_UnzerDirect_Instance extends WC_UnzerDirect {
	...
	public function generate_settings_html( $form_fields = array(), $echo = true ) {
		$main_settings_args = [
			'page'    => 'wc-settings',
			'tab'     => 'checkout',
			'section' => 'unzer_direct'
		];
		$html               = sprintf( "<p><small>Version: %s</small>", WC_UNZER_DIRECT_VERSION );
		$html               .= "<p>" . sprintf( __( 'Allows you to receive payments via %s', 'wc-unzer-direct' ), $this->method_title ) . "</p>";
		$html               .= "<p>" . sprintf( __( 'This module has it\'s main configuration inside the \'Unzer Direct\' tab.', 'wc-unzer-direct' ), 's' ) . "</p>";
		$html               .= "<p>" . sprintf( __( 'Click <a href="%s">here</a> to access the main configuration.', 'wc-unzer-direct' ), add_query_arg( $main_settings_args, admin_url( 'admin.php' ) ) ) . "</p>";
	...
	protected function get_sanitized_method_title() {
		return str_replace( 'Unzer Direct - ', '', $this->method_title );
	}

	/**
	 * @return bool
	 */
	protected function is_checkout_different_shipping_address_enabled() {
		parse_str( wc()->checkout()->get_value( 'post_data' ), $data );

		return isset( $data['ship_to_different_address'] ) && wc_string_to_bool( $data['ship_to_different_address'] );
	}
}
