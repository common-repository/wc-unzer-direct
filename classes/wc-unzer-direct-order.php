...
/**
 * WC_UnzerDirect_Order class
 *
 * @class          WC_UnzerDirect_Order
 * @version        1.1.0
 * @category       Class
 * @author         PerfectSolution
 */

class WC_UnzerDirect_Order extends WC_Order {

	/** */
	const META_PAYMENT_METHOD_CHANGE_COUNT = '_unzer_direct_payment_method_change_count';
	/** */
	const META_FAILED_PAYMENT_COUNT = '_unzer_direct_failed_payment_count';

	/**
	 * get_order_id_from_callback function.
	 *
	 * Returns the order ID based on the ID retrieved from the Unzer Direct callback.
	 *
	 ...
	 *
	 * Returns the subscription ID based on the ID retrieved from the Unzer Direct callback, if present.
	 *
	 * @access static public
	 *
	 * @param object - the callback data
	...
	public static function get_subscription_id_from_callback( $callback_data ) {
		...
		if ( isset( $_GET['subscription_post_id'] ) ) {
			return (int) $_GET['subscription_post_id'];
		}
	...
	public function get_payment_id() {
		return get_post_meta( $this->get_id(), 'UNZER_DIRECT_PAYMENT_ID', true );
	}
	...
	public function set_payment_id( $payment_link ) {
		update_post_meta( $this->get_id(), 'UNZER_DIRECT_PAYMENT_ID', $payment_link );
	}
	...
	public function delete_payment_id() {
		delete_post_meta( $this->get_id(), 'UNZER_DIRECT_PAYMENT_ID' );
	}
	...
	public function get_payment_link() {
		return get_post_meta( $this->get_id(), 'UNZER_DIRECT_PAYMENT_LINK', true );
	}
	...
	public function set_payment_link( $payment_link ) {
		update_post_meta( $this->get_id(), 'UNZER_DIRECT_PAYMENT_LINK', $payment_link );
	}
	...
	public function delete_payment_link() {
		delete_post_meta( $this->get_id(), 'UNZER_DIRECT_PAYMENT_LINK' );
	}
	...
	public function add_transaction_fee( $fee_amount ) {
		if ( $fee_amount > 0 ) {
			...
			try {
				...
				$fee->set_name( __( 'Payment Fee', 'wc-unzer-direct' ) );
				...
				$this->add_item( apply_filters( 'wc_unzer_direct_transaction_fee_data', $fee, $this ) );
	...
	public function subscription_is_renewal_failure() {
		...
		if ( WC_UnzerDirect_Subscription::plugin_is_active() ) {
			$renewal_failure = ( WC_UnzerDirect_Subscription::is_renewal( $this ) and $this->get_status() === 'failed' );
		}
	...
	public function note( $message ) {
		if ( isset( $message ) ) {
			$this->add_order_note( 'Unzer Direct: ' . $message );
		}
	}
	 ...
	 *
	 * Returns the necessary basic params to send to Unzer Direct when creating a payment
	 *
	 * @access public
	 * @return array
	 */
	public function get_transaction_params() {
		$is_subscription = $this->contains_subscription() || $this->is_request_to_change_payment() || WC_UnzerDirect_Subscription::is_subscription( $this->get_id() );
	...
	public function is_request_to_change_payment() {
		...
		if ( WC_UnzerDirect_Subscription::plugin_is_active() ) {
			...
			if ( ! $is_request_to_change_payment && ! empty( $_GET['unzer_direct_change_payment_method'] ) ) {
				$is_request_to_change_payment = true;
			}
		}

		return apply_filters( 'wc_unzer_direct_is_request_to_change_payment', $is_request_to_change_payment );
	}
	 ...
	 * because Unzer Direct requires the order number to contain at least
	 * 4 chars.
	 *
	 * @access public
	 *
	...
	public function get_order_number_for_api( $recurring = false ) {
		...
		if ( WC_UnzerDirect_Subscription::is_subscription( $order_id ) ) {
			$order_number = $order_id;
		} // On initial subscription authorizations
		else if ( ! $this->order_contains_switch() && $this->contains_subscription() && ! $recurring ) {
			// Find all subscriptions
			$subscriptions = WC_UnzerDirect_Subscription::get_subscriptions_for_order( $order_id );
			...
			if ( $this->get_failed_unzer_direct_payment_count() > 0 ) {
				$order_number .= sprintf( '-%d', $this->get_failed_unzer_direct_payment_count() );
			}
			...
			else if ( WC_UnzerDirect_Subscription::cart_contains_failed_renewal_order_payment() ) {
				// Get the last one and base the transaction on it.
				$subscription = WC_UnzerDirect_Subscription::get_subscriptions_for_renewal_order( $this->id, true );
			...
			else if ( WC_UnzerDirect_Subscription::cart_contains_renewal() ) {
		...
		return apply_filters( 'wc_unzer_direct_order_number_for_api', $order_number, $this, $recurring );
	}
	...
	/**
	 * Increase the amount of payment attemtps done through Unzer Direct
	 *
	 * @return int
	 */
	public function get_failed_unzer_direct_payment_count() {
		$order_id = $this->get_id();
	...
	/**
	 * Creates an array of order items formatted as "Unzer Direct transaction basket" format.
	 *
	 * @return array
	 */
	public function get_transaction_basket_params() {
		// Contains order items in Unzer Direct basket format
		$basket = [];
		...
		return apply_filters( 'wc_unzer_direct_transaction_params_basket', $basket, $this );
	}
	...
	private function get_transaction_basket_params_line_helper( $line_item ) {
		...
		return [
			...
			'item_price' => WC_UnzerDirect_Helper::price_multiply( $data['item_price'], $this->get_currency() ),
	...
	public function get_transaction_shipping_address_params() {
		...
		$params = [
			...
			'country_code'    => WC_UnzerDirect_Countries::getAlpha3FromAlpha2( $this->get_shipping_country() ),
			'phone_number'    => $this->get_billing_phone(),
			'mobile_number'   => $this->get_billing_phone(),
			'email'           => $this->get_billing_email(),
		];

		return apply_filters( 'wc_unzer_direct_transaction_params_shipping', $params, $this );
	}
	...
	public function get_shipping_street_name() {
		return WC_UnzerDirect_Address::get_street_name( $this->get_shipping_address_1() );
	}
	...
	public function get_shipping_house_number() {
		return WC_UnzerDirect_Address::get_house_number( $this->get_shipping_address_1() );
	}
	...
	public function get_shipping_house_extension() {
		return WC_UnzerDirect_Address::get_house_extension( $this->get_shipping_address_1() );
	}
	...
	public function get_transaction_invoice_address_params() {
		...
		$params = [
			...
			'country_code'    => WC_UnzerDirect_Countries::getAlpha3FromAlpha2( $this->get_billing_country() ),
			'phone_number'    => $this->get_billing_phone(),
			'mobile_number'   => $this->get_billing_phone(),
			'email'           => $this->get_billing_email(),
		];

		return apply_filters( 'wc_unzer_direct_transaction_params_invoice', $params, $this );
	}
	...
	public function get_billing_street_name() {
		return WC_UnzerDirect_Address::get_street_name( $this->get_billing_address_1() );
	}
	...
	public function get_billing_house_number() {
		...
		return WC_UnzerDirect_Address::get_house_number( $this->get_billing_address_1() );
	}
	...
	public function get_billing_house_extension() {
		return WC_UnzerDirect_Address::get_house_extension( $this->get_billing_address_1() );
	}
	...
	private function get_transaction_shipping_params() {
		...
		return apply_filters( 'wc_unzer_direct_transaction_params_shipping_row', [
			'method'          => 'own_delivery',
			'company'         => $this->get_shipping_method(),
			'amount'          => WC_UnzerDirect_Helper::price_multiply( $shipping_incl_vat, $this->get_currency() ),
	...
	public function get_transaction_shopsystem_params() {
		$params = [
			'name'    => 'WooCommerce',
			'version' => WC_UNZER_DIRECT_VERSION,
		];

		return apply_filters( 'wc_unzer_direct_transaction_params_shopsystem', $params, $this );
	}
	 ...
	 * be sent to Unzer Direct and stored with the transaction.
	 *
	 * @access public
	 * @return array
	 */
	public function get_custom_variables() {
		$custom_vars_settings = (array) WC_UNZER_DIRECT()->s( 'unzer_direct_custom_variables' );
		$custom_vars          = [];

		// Single: Order Email
		if ( in_array( 'customer_email', $custom_vars_settings ) ) {
			$custom_vars[ __( 'Customer Email', 'wc-unzer-direct' ) ] = $this->get_billing_email();
		}

		// Single: Order Phone
		if ( in_array( 'customer_phone', $custom_vars_settings ) ) {
			$custom_vars[ __( 'Customer Phone', 'wc-unzer-direct' ) ] = $this->get_billing_phone();
		}

		// Single: Browser User Agent
		if ( in_array( 'browser_useragent', $custom_vars_settings ) ) {
			$custom_vars[ __( 'User Agent', 'wc-unzer-direct' ) ] = $this->get_customer_user_agent();
		}

		// Single: Shipping Method
		if ( in_array( 'shipping_method', $custom_vars_settings ) ) {
			$custom_vars[ __( 'Shipping Method', 'wc-unzer-direct' ) ] = $this->get_shipping_method();
		}
		...
		if ( ! $this->order_contains_switch() ) {
			$subscription_id = WC_UnzerDirect_Subscription::get_subscription_id( $this );
			if ( $subscription_id ) {
		...
		$custom_vars = apply_filters( 'wc_unzer_direct_transaction_params_variables', $custom_vars, $this );
	 ...
	 * @return int
	 */
	public function increase_failed_unzer_direct_payment_count() {
		$order_id = $this->get_id();
		$count    = $this->get_failed_unzer_direct_payment_count();
	...
	/**
	 * Reset the failed payment attempts made through the Unzer Direct gateway
	 */
	public function reset_failed_unzer_direct_payment_count() {
		$order_id = $this->get_id();
	 ...
	 *
	 * Returns the necessary basic params to send to Unzer Direct when creating a payment link
	 *
	 * @access public
	 * @return array
	 */
	public function get_transaction_link_params() {
		...
		return [
			...
			'amount'      => WC_UnzerDirect_Helper::price_multiply( $amount, $this->get_currency() ),
		];
	}
	 ...
	 * and non-virtual products,  we will default to the 'unzer_direct_autocapture'-setting.
	 */
	public function get_autocapture_setting() {
		// Get the autocapture settings
		$autocapture_default = WC_UNZER_DIRECT()->s( 'unzer_direct_autocapture' );
		$autocapture_virtual = WC_UNZER_DIRECT()->s( 'unzer_direct_autocapture_virtual' );
		...
		// we use the 'unzer_direct_autopay' as the option of choice.
	...
	/**
	 * Checks if the order is paid with the Unzer Direct module.
	 *
	 * @return bool
	 * @since  4.5.0
	 * @access public
	 */
	public function has_unzer_direct_payment() {
		$order_id = $this->get_id();

		return strpos( get_post_meta( $order_id, '_payment_method', true ), 'unzer_' ) !== false;
	}
	...
	public function get_transaction_id( $context = 'view' ) {
		...
		$transaction_id = get_post_meta( $order_id, '_unzer_direct_transaction_id', true );
