<?php
/**
 * WC_UnzerDirect_Subscription class
 *
 * @class        WC_UnzerDirect_Subscription
 * @version        1.0.0
 * @category    Class
...
class WC_UnzerDirect_Subscription {
	...
	public static function get_initial_subscription_transaction_id( $order ) {
		...
		if ( $is_subscription ) {
			$original_order = new WC_UnzerDirect_Order( $order->post->post_parent );

			return $original_order->get_transaction_id();
		} else if ( self::is_renewal( $order ) ) {
			$subscriptions  = self::get_parent_order( $order );
			$subscription   = end( $subscriptions );
			$original_order = new WC_UnzerDirect_Order( $subscription->post->post_parent );
	 ...
	 *
	 * @param WC_UnzerDirect_Order $order [description]
	 *
	 * @return boolean
	 */
	...
	/**
	 * @param WC_UnzerDirect_Order $order The parent order
	 *
	 * @return bool
	 */
	public static function get_subscription_id( WC_UnzerDirect_Order $order ) {
		$order_id = $order->get_id();
