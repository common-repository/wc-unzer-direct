<?php
/**
 * WC_UnzerDirect_API class
 *
 * @class       WC_UnzerDirect_API
 * @since       4.0.0
 * @category    Class
...
class WC_UnzerDirect_API {
	 ...
	 * @access protected
	 */
	protected $api_url = 'https://api.unzerdirect.com/';
	...
	public function __construct( $api_key = null ) {
		if ( empty( $api_key ) ) {
			$this->api_key = WC_UNZER_DIRECT()->s( 'unzer_direct_apikey' );
		} else {
	...
	public function is_authorized_callback( $response_body ) {
		...
		return hash_hmac( 'sha256', $response_body, WC_UNZER_DIRECT()->s( 'unzer_direct_privatekey' ) ) == $_SERVER["HTTP_QUICKPAY_CHECKSUM_SHA256"];
	}
	...
	public function execute( $request_type, $form = [], $return_array = false ) {
		...
		if ( $response_code > 299 ) {
			if ( isset( $this->resource_data->errors ) ) {
				...
				throw new UnzerDirect_API_Exception( implode( "\n", $error_messages ), $response_code, null, $curl_request_url, $request_form_data, $response_data );
			} else if ( isset( $this->resource_data->message ) ) {
				throw new UnzerDirect_API_Exception( $this->resource_data->message, $response_code, null, $curl_request_url, $request_form_data, $response_data );
			} else {
				throw new UnzerDirect_API_Exception( (string) json_encode( $this->resource_data ), $response_code, null, $curl_request_url, $request_form_data, $response_data );
			}
	...
	protected function remote_instance( $post_id = null ) {
		...
		$callback_url = ! apply_filters( 'wc_unzer_direct_block_callback', false, $post_id ) ? WC_UnzerDirect_Helper::get_callback_url( $post_id ) : null;
